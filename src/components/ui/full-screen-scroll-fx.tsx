...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      if (bgRefs.current[0]) gsap.set(bgRefs.current[0], { opacity: 0.35, scale: 1 });
    ...
    const changeSection = (to: number) => {
      ...
      if (bgTransition === "fade") {
        if (newBg) {
          ...
          gsap.to(newBg, { opacity: 0.35, scale: 1, yPercent: 0, duration: D, ease: "power2.out" });
        }
        if (prevBg) {
          gsap.to(prevBg, {
            opacity: 0,
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-text" as any]: colors.text ?? "var(--foreground)",
      ["--fx-overlay" as any]: colors.overlay ?? "rgba(0,0,0,0.6)",
      ["--fx-page-bg" as any]: colors.pageBg ?? "transparent",
      ["--fx-stage-bg" as any]: colors.stageBg ?? "transparent",
    ...
    return (
      ...
      >
        ...
        <style jsx>{`
          ...
          .fx-bgs { 
             position: absolute; inset: 0; background: var(--fx-stage-bg); z-index: 1; 
          ...
          .fx-header {
            ...
            font-size: clamp(2rem, 9vw, 9rem); line-height: 0.86; text-align: center; color: var(--fx-text); font-weight: 800;
          }
          ...
          .fx-featured.active { opacity: 1; visibility: visible; text-transform: uppercase; }
          .fx-featured-title {
          ...
          .fx-footer-title { color: var(--fx-text); font-size: clamp(1rem, 2vw, 3rem); font-weight: 400; letter-spacing: -0.01em; line-height: 0.9; }
          ...
          @media (max-width: 900px) {
            ...
            .fx-left-item.active, .fx-right-item.active { padding: 0; }
