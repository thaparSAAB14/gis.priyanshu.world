...
import { FlaskConical, Sparkles, Beaker, Cpu, Maximize2, Minimize2 } from "lucide-react";
...
`
    ...
    const interval = setInterval(() => {
      if (Math.random() > 0.4) {
        setLogs(prev => {
          const isArt = Math.random() > 0.85; // 15% chance to roll an ASCII Art log
          const newLog = isArt 
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden transition-colors duration-300">
      ...
      />
      {/* Bottom blur mask for deep scroll readability */}
      ...
      <div className="relative z-10">
        ...
        <section className="relative w-full py-40 min-h-screen flex flex-col items-center justify-center z-20 px-4">
          ...
          <div 
            ...
            className={cn(
              "bg-card/20 backdrop-blur-3xl border border-foreground/10 rounded-2xl shadow-xl flex flex-col transition-all duration-500 overflow-hidden",
              showVideo
                ? "relative w-full max-w-5xl xl:max-w-6xl h-[70vh] min-h-[600px] lg:min-h-[800px] mt-8 z-30 ring-1 ring-foreground/20 shadow-primary/10"
                : "relative w-full max-w-3xl h-[450px] mt-8 z-20 ring-1 ring-foreground/5 hover:ring-foreground/20 transition-all shadow-black/20"
            )}
          >
            {/* Minimal Header */}
            <div className="h-12 bg-foreground/5 border-b border-foreground/5 flex items-center px-4 justify-between">
              ...
              <span className="absolute left-1/2 -translate-x-1/2 font-mono text-[10px] text-foreground/40 uppercase tracking-[0.2em] font-medium hidden sm:block">
                simulated_productivity.exe
              </span>

              <div className="flex items-center gap-2">
                <button
                  onClick={() => setShowVideo(!showVideo)}
                  className={cn("px-4 py-1.5 font-mono text-[10px] rounded-md transition-all border font-bold tracking-widest shadow-sm", showVideo ? "bg-primary/10 text-primary border-primary/30" : "text-foreground/50 border-transparent hover:bg-foreground/5 hover:text-foreground")}
                >
              ...
              {!showVideo ? (
                <div className="flex-1 bg-foreground/5 backdrop-blur-xl flex flex-col font-mono text-xs overflow-hidden shadow-2xl relative z-40">
                  ...
                  <div 
                    ...
                    className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 !scrollbar-thin !scrollbar-thumb-foreground/10"
                  >
                    {logs.map((log, i) => (
                      <motion.div
                        key={`${i}-${Math.random()}`} // Force fresh re-render to avoid key clipping on identical ASCIIs
                        initial={{ opacity: 0, y: -5 }}
                        animate={{ opacity: 1, y: 0 }}
                        className={cn(
                          "font-mono tracking-tight",
                          log.includes("\n") ? "whitespace-pre text-[10px] leading-tight opacity-70" : "leading-relaxed", 
                          log.includes("WARNING") || log.includes("ERROR") || log.includes("failed") || log.includes("down") ? "text-red-400/90" : log.includes("SUCCESS") || log.includes("[") ? "text-primary" : "text-foreground/60"
                        )}
                      >
                        {log}
                      </motion.div>
                  ...
                  <div className="p-3 border-t border-foreground/10 bg-foreground/5 text-foreground/30 text-[10px] flex items-center gap-2">
                    <span className="animate-pulse font-bold text-foreground/50">_</span> 
                    <span className="truncate">waiting for user competence...</span>
                  </div>
                </div>
              ) : (
        ...
        <footer className="py-12 px-4 border-t border-foreground/40">
          <div className="max-w-5xl mx-auto flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-foreground/70 ">
              made by{" "}
              <span className="text-primary font-medium">Priyanshu</span>{" "}
              <span className="italic text-foreground/70 ">with intentions</span>
            </p>
          </div>
        </footer>
      </div>
